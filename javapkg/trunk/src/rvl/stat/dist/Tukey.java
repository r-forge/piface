 ...
 */
    ...
    static final double M_1_SQRT_2PI = 0.398942280401432677939946059934;
...
/*  wprob() :
     ...
     */
        ...
        wprob = 2 * Normal.cdf(qsqz) - 1.; /* erf(qsqz / M_SQRT2) */
        /* if wprob ^ cc < 2e-22 then set wprob = 0 */
        if (wprob >= Math.exp(eps2 / cc))
            wprob = Math.pow(wprob, cc);
        else
            wprob = 0.0;
        ...
        for (wi = 1; wi <= wincr; wi++) {
            ...
            for (jj = 1; jj <= nleg; jj++) {
                ...
                pplus = 2 * Normal.cdf(ac);
                pminus= 2 * Normal.cdf(ac, w, 1);
        ...
        wprob = einsum + wprob;
        if (wprob <= Math.exp(eps1 / rr)) {
            return 0.;
        }
        wprob = Math.pow(wprob, rr);
        if (wprob >= eps)
            wprob = 1.;
        return wprob;
    } /* wprob() */
    ...
    public static double cdf(double q, double cc, double df, double rr) {
     ...
     */
        ...
        f2lf = ((f2 * Math.log(df)) - (df * M_LN2)) - MoreMath.logGamma(f2);
        f21 = f2 - 1.0;
    ...
    public static double cdf(double q, double cc, double df) {
        return cdf(q, cc, df, 1.0);
    }
 ...
 */
        ...
        double p4 = -0.453642210148e-04;
        double q4 = 0.38560700634e-02;
        double c1 = 0.8832;
        double c2 = 0.2368;
        double c3 = 1.214;
        double c4 = 1.208;
    ...
    public static double quantile(double p, double cc, double df, double rr) {
    /*
     *  Copenhaver, Margaret Diponzio & Holland, Burt S.
     *  Multiple comparisons of simple effects in
     *  the two-way analysis of variance with fixed effects.
 ...
 */
    public static double quantile(double p, double cc, double df) {
        return quantile(p, cc, df, 1.0);
    }
...
// test routine
    public static void main(String argv[]) {
        double q = quantile(.95, 5, 20);
        System.out.println("q = " + q);
        System.out.println("p = " + cdf(q, 5, 20));
    }
